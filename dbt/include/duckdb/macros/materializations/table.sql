{% materialization table, adapter="duckdb", supported_languages=['sql', 'python'] %}
  ...
  */
  ...
  {%- set use_ducklake_alter_workaround = adapter.use_ducklake_alter_workaround(target_relation) -%}
  ...
  {% if not use_ducklake_alter_workaround %}
  ...
  {% if use_ducklake_alter_workaround %}
    {#--
      Before DuckDB 1.5.3, DuckLake could lose table ALTER metadata during
      rename. Re-assert ALTERs on the final relation after commit.
      See https://github.com/duckdb/ducklake/pull/1154.
    --#}
    {% if partitioned_by %}
