...
class MotherDuckPgEndpointEnvironment(Environment):
    def __init__(self, credentials: credentials.DuckDBCredentials):
        ...
        if not self.creds.motherduck_postgres_endpoint:
    ...
    def _token(self):
        if self.creds.motherduck_token:
            return self.creds.motherduck_token

        parsed = urlparse(self.creds.path)
        query = parse_qs(parsed.query)
        if "motherduck_token" in query and query["motherduck_token"]:
            return query["motherduck_token"][0]
