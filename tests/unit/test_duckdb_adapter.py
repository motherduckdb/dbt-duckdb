...
class TestDuckDBAdapterShouldOmitDropCascade(unittest.TestCase):
    def setUp(self):
        set_from_args(Namespace(STRICT_MODE=True), {})
    ...
    def test_explicit_ducklake_relation_omits_cascade(self):
        profile_cfg = self.base_profile_cfg.copy()
        profile_cfg["outputs"]["test"]["attach"] = [
            {
                "alias": "ducklake_db",
        ...
        self.assertTrue(adapter.should_omit_drop_cascade(relation))

    def test_regular_relation_keeps_cascade(self):
        adapter = self._get_adapter(self.base_profile_cfg)
        relation = DuckDBRelation.create(
            database="memory",
            schema="test_schema",
        ...
        self.assertFalse(adapter.should_omit_drop_cascade(relation))

    def test_unqualified_relation_omits_cascade_when_target_is_ducklake(self):
        profile_cfg = self.base_profile_cfg.copy()
    ...
    def test_unqualified_relation_keeps_cascade_when_only_attachment_is_ducklake(self):
        profile_cfg = self.base_profile_cfg.copy()
        profile_cfg["outputs"]["test"]["attach"] = [
            {
                "alias": "ducklake_db",
